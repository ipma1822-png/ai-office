...
#newsList [data-article-text-copy]{border-color:rgba(231,191,99,.58)!important;background:rgba(231,191,99,.14)!important;color:var(--gold2)!important}
#newsList [data-image-request-copy]{border-color:rgba(94,162,255,.46)!important;background:rgba(94,162,255,.10)!important;color:#b9d7ff!important}
#newsList [data-caption-request-copy]{border-color:rgba(120,227,164,.42)!important;background:rgba(120,227,164,.09)!important;color:#9ce8bc!important}
...
function imageRequestMaterial(n){const url=sourceUrl(n);return ['[GN24 대표이미지 제작 요청 · v1.3]','기사 제목: '+(n?.title||''),'기사 요약: '+(n?.summary||''),'원문 링크: '+(url||'없음'),'','요청: 위 기사 원고에 사용할 GN24 대표이미지를 지금 생성해줘. 실제 인물 얼굴의 부정확한 재현은 피하고 사건·장소·상황 중심으로 제작해줘. 이미지 내부에는 한글·영문 제목이나 설명 문구를 넣지 마. GN24 공식 로고는 AI로 새로 그리거나 변형하지 말고 기존 공식 원본 로고를 후처리 합성하는 원칙을 지켜줘. 이 요청에서는 기사 원고를 다시 작성하지 말고 대표이미지 생성만 진행해줘.'].join('\n')}
function captionRequestMaterial(n){return ['[GN24 사진설명 작성 요청 · v1.3]','기사 제목: '+(n?.title||''),'','요청: 이 대화에서 방금 생성한 GN24 대표이미지를 직접 확인하고, 실제 이미지에 보이는 내용과 기사 맥락이 정확히 일치하도록 기사 하단에 사용할 사진설명글을 1~2문장으로 작성해줘. 이미지에 없는 사실이나 인물·장소를 단정하지 마.'].join('\n')}
...
function bindOnce(btn,key,handler){if(btn.dataset[key])return;btn.dataset[key]='1';btn.addEventListener('click',e=>{e.stopPropagation();handler()})}
function cleanCards(){const rows=read(),map=new Map(rows.map(n=>[String(n.id),n]));document.querySelectorAll('#newsList .news-item[data-news-id]').forEach(card=>{const n=map.get(String(card.dataset.newsId));if(!n)return;const small=card.querySelector('.news-item-main small');if(small){const url=sourceUrl(n),name=sourceName(n),date=shortDate(n.createdAt);small.innerHTML=url?`<a class="news-source-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(name)} 원문</a><span class="news-source-date">${esc(date)}</span>`:`<span>${esc(name)}</span><span class="news-source-date">${esc(date)}</span>`}const actions=card.querySelector('.news-item-actions');if(!actions)return;
let articleBtn=actions.querySelector('[data-news-copy-article],[data-article-material-copy],[data-article-text-copy]');if(!articleBtn){articleBtn=document.createElement('button');articleBtn.type='button';const first=actions.querySelector('button,a');if(first)first.insertAdjacentElement('afterend',articleBtn);else actions.appendChild(articleBtn)}articleBtn.removeAttribute('data-news-copy-article');articleBtn.removeAttribute('data-article-material-copy');articleBtn.setAttribute('data-article-text-copy','');articleBtn.textContent='기사원고 복사 v1.3';bindOnce(articleBtn,'v13Article',()=>copyText(articleBtn,articleTextMaterial(n)));
let imageBtn=actions.querySelector('[data-image-request-copy]');if(!imageBtn){imageBtn=document.createElement('button');imageBtn.type='button';imageBtn.setAttribute('data-image-request-copy','');imageBtn.textContent='대표이미지 요청 복사';articleBtn.insertAdjacentElement('afterend',imageBtn)}bindOnce(imageBtn,'v13Image',()=>copyText(imageBtn,imageRequestMaterial(n)));
let captionBtn=actions.querySelector('[data-caption-request-copy]');if(!captionBtn){captionBtn=document.createElement('button');captionBtn.type='button';captionBtn.setAttribute('data-caption-request-copy','');captionBtn.textContent='사진설명 요청 복사';imageBtn.insertAdjacentElement('afterend',captionBtn)}bindOnce(captionBtn,'v13Caption',()=>copyText(captionBtn,captionRequestMaterial(n)));
})}

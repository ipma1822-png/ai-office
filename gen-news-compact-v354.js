...
#newsList [data-article-material-copy]{border-color:rgba(231,191,99,.48)!important;background:rgba(231,191,99,.12)!important;color:var(--gold2)!important}
...
function articleMaterial(n){const url=sourceUrl(n);return ['[GN24 기사 제작 자료]','제목: '+(n?.title||''),'현재 GEN 분야: '+(n?.category||'종합'),'출처: '+(n?.source||sourceName(n)||'출처 미입력'),'요약: '+(n?.summary||''),'원문 링크: '+(url||'없음'),'','요청: 이 자료와 원문 및 필요한 추가 출처를 확인하여 사실관계를 검증한 뒤 GLOBAL NEWS24용 독립기사로 재작성해줘. 제목·부제목·요약·본문을 완성하고, GN24 메뉴 기준 가장 적합한 추천 분류와 검색·노출에 적합한 추천 태그도 함께 제시해줘. 현재 GEN 분야는 참고값일 뿐이며 원문 내용에 따라 더 적합한 분류를 판단해줘. 기사 작성이 끝나면 기사 내용에 적합한 GN24 대표이미지를 생성해줘. 실제 인물 얼굴의 부정확한 재현은 피하고 사건·장소·상황 중심의 이미지로 제작하며, GN24 공식 로고는 AI로 새로 그리거나 변형하지 말고 기존 공식 원본 로고를 후처리 합성하는 원칙을 지켜줘. 이미지 생성이 완료된 뒤 실제 생성된 이미지 내용에 정확히 맞는 사진설명글까지 작성해줘.'].join('\n')}
async function copyArticleMaterial(btn,n){const text=articleMaterial(n);try{await navigator.clipboard.writeText(text);const old=btn.textContent;btn.textContent='복사 완료';setTimeout(()=>{if(btn.isConnected)btn.textContent=old},1400)}catch(_){window.prompt('아래 기사자료를 복사하세요.',text)}}
function cleanCards(){const rows=read(),map=new Map(rows.map(n=>[String(n.id),n]));document.querySelectorAll('#newsList .news-item[data-news-id]').forEach(card=>{const n=map.get(String(card.dataset.newsId));if(!n)return;const small=card.querySelector('.news-item-main small');if(small){const url=sourceUrl(n),name=sourceName(n),date=shortDate(n.createdAt);small.innerHTML=url?`<a class="news-source-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(name)} 원문</a><span class="news-source-date">${esc(date)}</span>`:`<span>${esc(name)}</span><span class="news-source-date">${esc(date)}</span>`}const actions=card.querySelector('.news-item-actions');if(actions&&!actions.querySelector('[data-article-material-copy]')){const btn=document.createElement('button');btn.type='button';btn.textContent='기사자료 복사';btn.setAttribute('data-article-material-copy','');btn.addEventListener('click',()=>copyArticleMaterial(btn,n));const first=actions.querySelector('button,a');if(first)first.insertAdjacentElement('afterend',btn);else actions.appendChild(btn)}})}

...
function articleMaterial(n){const url=sourceUrl(n);return ['[GN24 기사 제작 자료]','제목: '+(n?.title||''),'현재 GEN 분야: '+(n?.category||'종합'),'출처: '+(n?.source||sourceName(n)||'출처 미입력'),'요약: '+(n?.summary||''),'원문 링크: '+(url||'없음'),'','요청: 이 자료와 원문을 확인하여 GN24용 제목·부제목·요약·본문·사진설명을 작성하고, GN24 메뉴 기준 추천 분류와 검색·노출에 적합한 추천 태그도 함께 제시해줘. 현재 GEN 분야는 참고값일 뿐이며 원문 내용에 따라 더 적합한 분류를 판단해줘.'].join('\n')}

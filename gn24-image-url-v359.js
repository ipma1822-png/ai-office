...
const AUTO_IMAGE_URL='https://news24.ai.kr/assets/images/news/2026/09/gn24-public-servant-suicide-prevention-20260908.png';
...
function loadCurrent(){
 ...
 if(isCurrentTestArticle(a)&&(!url||url.endsWith('.svg'))){url=AUTO_IMAGE_URL;$('nr359ImageUrl').value=url;persist();return;}
...
function tick(){install();loadCurrent();const id=$('nr350Id')?.value||'';if(id&&$('nr359ImageUrl')){const a=article(id);if(a&&isCurrentTestArticle(a)&&(!a.imageUrl||String(a.imageUrl).endsWith('.svg'))){$('nr359ImageUrl').value=AUTO_IMAGE_URL;persist();return;}if(a&&a.imageUrl!==$('nr359ImageUrl').value&&document.activeElement!==$('nr359ImageUrl')){$('nr359ImageUrl').value=a.imageUrl||'';renderStatus(a.imageUrl||'')}}}

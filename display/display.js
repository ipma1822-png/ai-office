...
async function boot(){if(!await requireSession("../",true))return;try{await refreshCatalog()}catch(e){console.warn("AI OFFICE catalog unavailable",e);state.contents=[];$("pairMessage").textContent="자료 목록은 나중에 불러옵니다. 연결코드는 사용할 수 있습니다."}db.channel("ai-office-catalog-display").on("postgres_changes",{event:"*",schema:"public",table:"ai_office_contents"},()=>refreshCatalog().catch(()=>{})).subscribe();const saved=localStorage.getItem("aiOfficeDisplayCode");if(saved)connect(saved);$("pairForm").onsubmit=e=>{e.preventDefault();const code=clean($("code").value);if(!/^\d{6}$/.test(code)){$("pairMessage").textContent="6자리 숫자를 입력해 주세요.";return}connect(code)};$("fullscreen").onclick=()=>document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen?.();$("connection").onclick=()=>{if(state.channel)db.removeChannel(state.channel);$("pairing").style.display="grid";$("code").value=state.code}}boot();
